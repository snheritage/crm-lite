...
async def admin_stats(
    ...
    by_cemetery_q = (
        select(Monument.cemetery_name, func.count().label("count"))
        .group_by(Monument.cemetery_name)
    ...
    # Monuments by user
    by_user_q = (
        select(User.email, User.full_name, func.count(Monument.id).label("count"))
        .join(Monument, Monument.user_id == User.id)
    ...
    by_month_q = (
        select(
            func.to_char(Monument.created_at, 'YYYY-MM').label("month"),
            func.count().label("count"),
        )
        .group_by(func.to_char(Monument.created_at, 'YYYY-MM'))
        .order_by(func.to_char(Monument.created_at, 'YYYY-MM').desc())
    )
    by_month_rows = (await db.execute(by_month_q)).all()
    monuments_by_month = [
        MonthCount(month=row[0], count=row[1]) for row in by_month_rows
    ]

    # Recent monuments (last 10) with user info
    recent_q = (
        select(Monument, User.email, User.full_name)
